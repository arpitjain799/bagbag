...
def GetPrice(pair:str|list=None) -> CoinsPairPrice | list[CoinsPairPrice]:
    ...
    if pair != None:
        if type(pair) == str:
            ...
            url = url + "?symbols=" + String(pair).URLEncode()  
        else:
    ...
    if pair != None and type(pair) == str:
        return CoinsPairPrice(content['symbol'], content['price'], ServerTime())

    resp = []
    for p in content:
        resp.append(CoinsPairPrice(p['symbol'], p['price'], ServerTime()))
